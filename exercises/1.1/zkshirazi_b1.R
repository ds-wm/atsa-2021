...
cat('The mean of realization is between t = 0 and t = 100: ', mean.realization)
# Output
# mean of realization between t = 0 and t = 100:  -0.005063656

# Question 3 
# The ensemble mean refers to the expected value or the average of all realizations at a single moment. 
# In applied time series analysis, to determine the mean, we need to apply time 't' into 'n' number of realizations.
# Then determine the weighted average of all realization outputs by applying a different time into each realization. 
# The mean of a realization refers to the set of values that is observed during a time series.
# Using integration, one can determine the average value over a time interval. 
