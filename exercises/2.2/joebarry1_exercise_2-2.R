...
# Now we want to calculate our product, Qgap
# To do this, we need to extract fFEC, SWdown, I_0, and H_0
# for each day in our month of interest, which is Dec. 2003.

#SWdown is stored in:
SWdown <- watch.df$SWdown_2003.12*86400

#fFEC is a constant conversion factor which was provided
fFEC <- 2.04

# I_0 and H_0 will be calculated using our functions above for daily solar and fluxes
# Can use lat, lon, and elv from above as those referred to the same point
I_0 <- numeric(0)
H_0 <- numeric(0)

for (i in 1:31) {
  day <- julian_day(2003, 12, i) - (julian_day(2003, 1, 1) + 1) #To get day of the year
  
  io.calc <- calc_daily_fluxes(day,
                               lat = my.lat,
                               lon= my.lon,
                               elv = my.elv,
                               y = 2003)
  I_0 <- c(I_0, io.calc$io_wm2)
  
  ho.calc <- calc_daily_solar(lat = my.lat,
                              n = day,
                              elv = my.elv,
                              y = 2003)
  H_0 <- c(H_0, ho.calc$ra_j.m2)
}

I_0
H_0

# We want to extend H_0 and SWdown so that they are half-hourly instead of daily
# We will just create new lists with 48 entries for each one in these datasets
...
# Now that we have all of our values in 1488 entry lists,
# we can calculate all 1488 values for Qgap in our month

Qgap <- (sw.cmpl/ho.cmpl)*I_0*fFEC
Qgap
...
plot(
  ...
  type = 'l', 
  xlab = "Half-hourly samples", 
  ylab = bquote("Estimated PPFD " ~ mu ~ "mol" ~ m^{-2} ~ s^{-1}),
  main = "Q_gap calculated Photosynthetic Radiation Estimate, Dec. 2003"
)

# We have our replacement data!
# Now we have to overlay the data with what we have.
# The easiest way to do this seems to be creating a df
# which is NA where flux12.ppfd is not and plotting both

i <- 1
Qplot <- numeric(0)
for (j in Qgap) {
  if (is.na(flux12.ppfd[i])) {
    Qplot <- c(Qplot, Qgap[i])
  }
  else {
    Qplot <- c(Qplot, NA)
  }
...
# Now we just have to plot this and flux12.ppfd together
...
plot(
  ...
  xlab = "Half-hourly samples", 
  ylab = bquote("Estimated PPFD " ~ mu ~ "mol" ~ m^{-2} ~ s^{-1}),
  main = "Gap-filled Photosynthetic Radiation, Dec. 2003"
)

lines(flux12.ppfd, type = 'l', col = 'blue')
