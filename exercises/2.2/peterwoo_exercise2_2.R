...
# ---------------------------------------------------------------------------
# Read WATCH data and FLUXNET data
...
for (i in seq(1,31)){
  ...
  SWdown <- watch.df[i,'SWdown_2003.12'] * 86400 # 86400 is the number of seconds in a day 
  H0 <- calc_daily_solar(lat = my.lat, n = n, elv = my.elv, y = 2003)$ra_j.m2 # Information pulled from http://sites.fluxdata.org/US-Goo/
  I0 <- calc_daily_fluxes(lat = my.lat, lon = my.lon, n = n, elv = my.elv, y = 2003)$io_wm2
  Qgap <- kfFEC * (SWdown/H0) * I0
  gap_fills <- c(gap_fills, Qgap)
}

# 3. Overlay FLUXNET data ontop of gap-filling product

data_filled <- c()
for (i in seq(1, length(flux12.ppfd))){
  if(is.na(flux12.ppfd)){
    data_filled <- c(data_filled, gap_fills[i])
  }
  else{
    data_filled <- c(data_filled, flux12.ppfd[i])
  }
}

# Turning data into time series
data_filled.ts <- ts(data_filled)
data_gaps.ts <- ts(flux12.ppfd)
...
# Complete data with the gaps filled in
