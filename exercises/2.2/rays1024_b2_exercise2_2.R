...
# Read data
flux.url <- "https://ds-wm.github.io/course/atsa/data/FLX_US-Goo_FLUXNET2015_SUBSET_HH_2003-12_1-4.csv"
flux.df <- read.csv(flux.url)
...
flux.ppfd <- flux.df$PPFD_IN
flux.ppfd[which(flux.ppfd == -9999)] <- NA
...
plot(
  ...
  main = "Measured Photosynthetic Radiation - AmeriFlux US-Goo (2002-05)"
)
...
# Subset df for 2002, the year with gap filled values
...
# Gap fill values are in December, which has 31 days. 
...
gaps <- c()
for (i in 1:length(flux.ppfd)){
  if (is.na(flux.ppfd[i])){
...
gaps_ts <- ts(gaps)
flux.ppfd_ts <- ts(flux.ppfd)

# 3. Overlay FLUXNET data ontop of gap-filling product
options(repr.plot.width=14, repr.plot.height=10, repr.plot.res = 125)
plot(flux.ppfd_ts, type = 'l', col = "blue", ylim = c(0,1200), xlab = 'Time', ylab = 'FLUX', main = 'Gap-Filled Solar Radiation Data')
lines(gaps_ts, col = "red", lty = 3)
legend(450, 1200, legend=c("Gap-Filled Values", "Original Values"),
