# Exercise 2.2, ATSA
# John Hennin, 2021-04-9


# ---------------------------------------------------------------------------
#                    Beginning of Dr. Davis' Code Snippet
# ---------------------------------------------------------------------------
...
# Make dataframe for Io using calc_daily_fluxes
...
# Initialize dataframe for all variables
df.all <- data.frame(matrix(nrow=1488, ncol = 5))
names(df.all) <-c("Day", "Io", "Ho", "SWd", "Qg")

# Copy over Day and Io columns from df.io (same # of observations)
...
for (n in 1:1488){
  # Pair Ho values with corresponding day of year for each half hour of Dec.
  df.all$Ho[n] <- df.ho$Ho[which(df.ho$Day==df.all$Day[n])]
  
  # Pair SWd values with corresponding day of year for each half hour of Dec.
  df.all$SWd[n] <- df.sw$SWd[which(df.sw$Day==df.all$Day[n])]
  
  # Calculate Qg value for corresponding half hour of December
  df.all$Qg[n] <- (df.all$SWd[n]/df.all$Ho[n])*df.all$Io[n]*2.04
  
  # Fill gaps in original data that has been copied over to filled.df
