...
day_diff <- julian_day(2020,3,25) - julian_day(2020,1,1)+1

out <- calc_daily_solar(34.2547, day_diff, elv = 87, y=2003)
h0 <- out$ra_j.m2

#swdown 
# watch.df$SWdown_2002.05
watch.df$SWdown_2003.12 # <---- this is the one we are working with

#f_FEC 
ffec <- 2.04 #mu mol/J

# We need to first calculate the gap fill values
# which are in December of 2003
# So, we iteratively calculate these values by 
# finding little n for each day from December 1st
# to December 31st and calculate the Solar fluxes for
# each of these days in temp_store 

temp_store <- numeric()
for (i in 1:31){
  n <- julian_day(2003, 12, i) - julian_day(2003, 1, 1)+1
  
  out <- calc_daily_fluxes(n, lat= 34.2547, lon=-89.8735, elv = 87, y=2003)
  I_0 <- out$io_wm2
  
  out <- calc_daily_solar(34.2547, n, elv=87, y=2003)
  h_0 <- out$ra_j.m2
  
  swdown <- watch.df$SWdown_2003.12[i]*86400
  
  q_gap <- ffec*(swdown/h_0)*I_0
  
  temp_store <- c(temp_store, q_gap)
  
  # temp_store[[i]] <- q_gap
  # I tried to use list indexing but ran into duplication
  # issues...decided to see how other students addressed 
  # the iterative storing of values and saw vector-creation
  # to be a popular fix!
}


# Next, I am creating dataframes that are of the same length
# as the original data frame but with the missing values 
# that I recalculated above added into the data frame
# and holes everywhere where we had valid data...this is done
# so we can have the delineation between blue and red lines
gaps <- numeric()
full <- numeric()
...
for (i in 1:length(flux12.ppfd)){
  ...
  {
    gaps <- c(gaps,temp_store[i])
    full <- c(gaps,temp_store[i])
  }
  else
  {
    gaps <- c(gaps,NA)
...
# Define each data set as a time series object
# and ensure that the x-axis values are representative
# of the original plot we have with the missing data
fq <- 1
ppfd_gapfill <- ts(gaps, frequency = fq, start = 0)
ppfd_orig <- ts(flux12.ppfd, frequency = fq, start = 0)
ppfd_full <- ts(full, frequency = fq, start = 0) 


# set options so we have a better view of the plot
options(repr.plot.width = 18, repr.plot.height = 6, repr.plot.res = 125)


# finally, we plot our data by first creating an empty 
# plot with axes corresponding to the original plot
# then filling the original data as blue and the 
# calculated missing values as red lines.

plot(ppfd_full, type = 'n', xaxt = 'n', xlab = 'Half-hourly samples', ylab =  bquote("PPFD " ~ mu ~ "mol" ~ m^{-2} ~ s^{-1}), main = "Missing Photosynthetic Radiation - AmeriFlux US-Goo (2003-12)")

axis(1, at = seq(0, 1500, by = 100))

lines(ppfd_orig, lty = 2, col = 'blue')
lines(ppfd_gapfill, lty = 2, col = 'red')
